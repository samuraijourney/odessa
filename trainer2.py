...
if __name__ == '__main__':
    f1 = open('final_trainer.txt', 'w')
    f2 = open('final_results.csv', 'w')
    original = sys.stdout
    ...
    hmm_folder_path = "C:\Users\AkramAsylum\OneDrive\Courses\School\EE 516 - Compute Speech Processing\Assignments\Assignment 5\hmm_final"
    ...
    if not os.path.exists(odessa_hmm):
        training_list.append([odessa_validation_samples, odessa_samples, odessa_hmm, 14])
    if not os.path.exists(play_music_hmm):
        training_list.append([play_music_validation_samples, play_music_samples, play_music_hmm, 14])
    if not os.path.exists(stop_music_hmm):
        training_list.append([stop_music_validation_samples, stop_music_samples, stop_music_hmm, 18])
    if not os.path.exists(turn_on_the_lights_hmm):
        training_list.append([turn_on_the_lights_validation_samples, turn_on_the_lights_samples, turn_on_the_lights_hmm, 26])
    if not os.path.exists(turn_off_the_lights_hmm):
        training_list.append([turn_off_the_lights_validation_samples, turn_off_the_lights_samples, turn_off_the_lights_hmm, 22])
    if not os.path.exists(what_time_is_it_hmm):
        training_list.append([what_time_is_it_validation_samples, what_time_is_it_samples, what_time_is_it_hmm, 30])
    ...
    for item in training_list:
        for iteration in range(0, 10):
            ...
            hmm_path = "%s_%d.hmm" % (os.path.splitext(item[2])[0], iteration)
            if os.path.exists(hmm_path):
                print("Skipping already trained %s for i=%d..." % (folder_name, iteration))
                continue
            print("Training %s for i=%d..." % (folder_name, iteration))
            
            speech_hmm = em.build_hmm_from_folder(item[1], item[3], max_iterations = 250, show_plots = False, convergence_threshold = 0.05)
            ...
            csv.write([folder_name, iteration, "training"] + metrics)
            ...
            metrics = [max(matches), min(matches), np.mean(matches), np.std(matches), np.percentile(matches, 5), np.percentile(matches, 10), np.percentile(matches, 15), np.percentile(matches, 20)]
            csv.write([folder_name, iteration, "validation"] + metrics)
            print("\tmax: %.3f, min: %.3f, mean: %.3f, std: %.3f, cutoff: %.3f" % (metrics[0], metrics[1], metrics[2], metrics[3], metrics[4]))
            ...
            if garbage_validation_samples != item[1]:
                ...
                csv.write([folder_name, iteration, "garbage"] + metrics)
            ...
            if odessa_samples != item[1]:
                ...
                metrics = [max(matches), min(matches), np.mean(matches), np.std(matches), np.percentile(matches, 95), np.percentile(matches, 90), np.percentile(matches, 85), np.percentile(matches, 80)]
                csv.write([folder_name, iteration, "odessa"] + metrics)
                print("\tmax: %.3f, min: %.3f, mean: %.3f, std: %.3f" % (metrics[0], metrics[1], metrics[2], metrics[3]))
            ...
            if play_music_samples != item[1]:
                ...
                metrics = [max(matches), min(matches), np.mean(matches), np.std(matches), np.percentile(matches, 95), np.percentile(matches, 90), np.percentile(matches, 85), np.percentile(matches, 80)]
                csv.write([folder_name, iteration, "play music"] + metrics)
                print("\tmax: %.3f, min: %.3f, mean: %.3f, std: %.3f" % (metrics[0], metrics[1], metrics[2], metrics[3]))
            ...
            if stop_music_samples != item[1]:
                ...
                metrics = [max(matches), min(matches), np.mean(matches), np.std(matches), np.percentile(matches, 95), np.percentile(matches, 90), np.percentile(matches, 85), np.percentile(matches, 80)]
                csv.write([folder_name, iteration, "stop music"] + metrics)
                print("\tmax: %.3f, min: %.3f, mean: %.3f, std: %.3f" % (metrics[0], metrics[1], metrics[2], metrics[3]))
            ...
            if turn_on_the_lights_samples != item[1]:
                ...
                metrics = [max(matches), min(matches), np.mean(matches), np.std(matches), np.percentile(matches, 95), np.percentile(matches, 90), np.percentile(matches, 85), np.percentile(matches, 80)]
                csv.write([folder_name, iteration, "turn on the lights"] + metrics)
                print("\tmax: %.3f, min: %.3f, mean: %.3f, std: %.3f" % (metrics[0], metrics[1], metrics[2], metrics[3]))
            ...
            if turn_off_the_lights_samples != item[1]:
                ...
                metrics = [max(matches), min(matches), np.mean(matches), np.std(matches), np.percentile(matches, 95), np.percentile(matches, 90), np.percentile(matches, 85), np.percentile(matches, 80)]
                csv.write([folder_name, iteration, "turn off the lights"] + metrics)
                print("\tmax: %.3f, min: %.3f, mean: %.3f, std: %.3f" % (metrics[0], metrics[1], metrics[2], metrics[3]))
            ...
            if what_time_is_it_samples != item[1]:
                ...
                metrics = [max(matches), min(matches), np.mean(matches), np.std(matches), np.percentile(matches, 95), np.percentile(matches, 90), np.percentile(matches, 85), np.percentile(matches, 80)]
                csv.write([folder_name, iteration, "what time is it"] + metrics)
                print("\tmax: %.3f, min: %.3f, mean: %.3f, std: %.3f" % (metrics[0], metrics[1], metrics[2], metrics[3]))
